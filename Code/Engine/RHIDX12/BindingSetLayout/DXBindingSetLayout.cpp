...
DXBindingSetLayout::DXBindingSetLayout(DXDevice& device, const ezDynamicArray<BindKey>& descs)
  : m_device(device)
{
  ...
  auto add_root_table = [&](ShaderType shader_type, ezUInt32 range_count, const D3D12_DESCRIPTOR_RANGE* ranges) {
    ...
    ezUInt32 root_param_index = (ezUInt32)root_parameters.size();
  ...
  auto add_bindless_range = [&](ShaderType shader_type, ViewType view_type, ezUInt32 base_slot, ezUInt32 space) {
    ...
    ezUInt32 root_param_index = add_root_table(shader_type, 1, &descriptor_table_range);
  ...
  EZ_ASSERT_ALWAYS(D3D12SerializeRootSignature(&root_signature_desc, D3D_ROOT_SIGNATURE_VERSION_1, &signature, &error_blob) == S_OK,
