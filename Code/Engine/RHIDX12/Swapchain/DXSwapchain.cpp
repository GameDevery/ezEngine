...
ezUInt32 DXSwapchain::NextImage(const ezSharedPtr<Fence>& fence, ezUInt64 signal_value)
{
  ezUInt32 frameIndex = m_SwapChain->GetCurrentBackBufferIndex();
  m_CommandQueue.Signal(fence, signal_value);
  return frameIndex;
}

void DXSwapchain::Present(const ezSharedPtr<Fence>& fence, ezUInt64 wait_value)
{
  m_CommandQueue.Wait(fence, wait_value);
  if (m_vsync)
  {
    EZ_ASSERT_ALWAYS(m_SwapChain->Present(1, 0) == S_OK, "");
  }
  else
  {
    EZ_ASSERT_ALWAYS(m_SwapChain->Present(0, DXGI_PRESENT_ALLOW_TEARING) == S_OK, "");
  }
}
