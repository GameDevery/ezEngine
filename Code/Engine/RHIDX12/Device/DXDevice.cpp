...
{
  EZ_ASSERT_ALWAYS(D3D12CreateDevice(m_adapter.GetAdapter().Get(), D3D_FEATURE_LEVEL_11_1, IID_PPV_ARGS(&m_device)) == S_OK, "");
  ...
  if (debug_enabled)
  {
    ...
    if (SUCCEEDED(m_device.As(&info_queue)))
    {
      ...
      filter.DenyList.NumSeverities = EZ_ARRAY_SIZE(severities);
      filter.DenyList.pSeverityList = severities;
      filter.DenyList.NumIDs = EZ_ARRAY_SIZE(deny_ids);
...
std::shared_ptr<Resource> DXDevice::CreateSampler(const SamplerDesc& desc)
{
  ...
  sampler_desc.MaxLOD = ezMath::MaxValue<float>();
...
ID3D12CommandSignature* DXDevice::GetCommandSignature(D3D12_INDIRECT_ARGUMENT_TYPE type, uint32_t stride)
{
  ...
  EZ_ASSERT_ALWAYS(m_device->CreateCommandSignature(
                     &desc,
                     nullptr,
