...
{
  return type >= Type::SAMPLER1D && type <= Type::SAMPLER_CUBE;
}
bool IsSampledImage(Type type)
{
  return type >= Type::TEXTURE1D && type <= Type::TEXTURE_CUBE;
}
bool IsStorageImage(Type type)
{
  return type >= Type::IMAGE1D && type <= Type::IMAGE_CUBE;
}

ezUInt32 CeilDiv(ezUInt32 x, ezUInt32 y)
{
  return (x - 1) / y + 1;
}

ezUInt32 FormatSize(Format format, ezUInt32 width, ezUInt32 height, ezUInt32 depth)
{
  if (!GFX_FORMAT_INFOS[static_cast<ezUInt32>(format)].isCompressed)
  {
    return (width * height * depth * GFX_FORMAT_INFOS[static_cast<ezUInt32>(format)].size);
  }
  switch (format)
  {
    case Format::BC1:
    ...
    case Format::PVRTC2_2BPP:
      return CeilDiv(ezMath::Max(width, 16U) * ezMath::Max(height, 8U), 4) * depth;
    case Format::PVRTC_RGB4:
    case Format::PVRTC_RGBA4:
    case Format::PVRTC2_4BPP:
      return CeilDiv(ezMath::Max(width, 16U) * ezMath::Max(height, 8U), 2) * depth;
...
ezUInt32 formatSurfaceSize(Format format, ezUInt32 width, ezUInt32 height, ezUInt32 depth, ezUInt32 mips)
{
  ezUInt32 size = 0;

  for (ezUInt32 i = 0; i < mips; ++i)
  {
    size += FormatSize(format, width, height, depth);
    width = ezMath::Max(width >> 1, 1U);
    height = ezMath::Max(height >> 1, 1U);
  }
