...
namespace
{
  ...
  constexpr ezUInt32 FORCE_MINOR_VERSION = 0;           // 0 for default version, otherwise minorVersion = (FORCE_MINOR_VERSION - 1)
  constexpr ezUInt32 DISABLE_VALIDATION_ASSERTIONS = 1; // 0 for default behavior, otherwise assertions will be disabled
...
bool CCVKGPUContext::initialize()
{
  ...
  requestedLayers.PushBack("VK_LAYER_KHRONOS_synchronization2");

  ezDynamicArray<const char*> requestedExtensions;
  requestedExtensions.PushBack(VK_KHR_SURFACE_EXTENSION_NAME);
  ...
  ezUInt32 apiVersion = VK_API_VERSION_1_0;
  ...
  if (minorVersion < 1)
  {
    requestedExtensions.PushBack(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME);
  }

  ezUInt32 availableLayerCount;
  VK_CHECK(vkEnumerateInstanceLayerProperties(&availableLayerCount, nullptr));
  ezDynamicArray<VkLayerProperties> supportedLayers;
  supportedLayers.SetCount(availableLayerCount);
  VK_CHECK(vkEnumerateInstanceLayerProperties(&availableLayerCount, supportedLayers.GetData()));

  ezUInt32 availableExtensionCount;
  VK_CHECK(vkEnumerateInstanceExtensionProperties(nullptr, &availableExtensionCount, nullptr));
  ezDynamicArray<VkExtensionProperties> supportedExtensions;
  supportedExtensions.SetCount(availableExtensionCount);
  VK_CHECK(vkEnumerateInstanceExtensionProperties(nullptr, &availableExtensionCount, supportedExtensions.GetData()));

#if defined(VK_USE_PLATFORM_ANDROID_KHR)
  requestedExtensions.PushBack(VK_KHR_ANDROID_SURFACE_EXTENSION_NAME);
#elif defined(VK_USE_PLATFORM_WIN32_KHR)
  requestedExtensions.PushBack(VK_KHR_WIN32_SURFACE_EXTENSION_NAME);
#elif defined(VK_USE_PLATFORM_METAL_EXT)
  requestedExtensions.PushBack(VK_EXT_METAL_SURFACE_EXTENSION_NAME);
#elif defined(VK_USE_PLATFORM_WAYLAND_KHR)
  requestedExtensions.PushBack(VK_KHR_WAYLAND_SURFACE_EXTENSION_NAME);
#elif defined(VK_USE_PLATFORM_XCB_KHR)
  requestedExtensions.PushBack(VK_KHR_XCB_SURFACE_EXTENSION_NAME);
#else
#  pragma error Platform not supported
  ...
  instanceInfo.enabledExtensionCount = extensions.GetCount();
  instanceInfo.ppEnabledExtensionNames = extensions.GetData();
  instanceInfo.enabledLayerCount = layers.GetCount();
  ...
  {
    ezLog::Info("Create Vulkan instance failed due to missing layers, aborting...");
    return false;
  }
  ...
  ezDynamicArray<VkPhysicalDevice> physicalDeviceHandles;
  physicalDeviceHandles.SetCount(physicalDeviceCount);
  VK_CHECK(vkEnumeratePhysicalDevices(vkInstance, &physicalDeviceCount, physicalDeviceHandles.GetData()));

  ezDynamicArray<VkPhysicalDeviceProperties> physicalDevicePropertiesList;
  physicalDevicePropertiesList.SetCount(physicalDeviceCount);

  ezUInt32 deviceIndex;
  ...
  ezUInt32 queueFamilyPropertiesCount = 0;
  vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, &queueFamilyPropertiesCount, nullptr);
  queueFamilyProperties.SetCount(queueFamilyPropertiesCount);
  vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, &queueFamilyPropertiesCount, queueFamilyProperties.GetData());
