...
VkFormat mapVkFormat(Format format, const CCVKGPUDevice* gpuDevice)
{
  switch (format)
  {
    ...
    case Format::DEPTH:
      return gpuDevice->depthFormat;
    case Format::DEPTH_STENCIL:
      return gpuDevice->depthStencilFormat;
    ...
    default:
    {
      EZ_ASSERT_DEV(false, "Unsupported Format, convert to VkFormat failed.");
      return VK_FORMAT_B8G8R8A8_UNORM;
    }
  }
}
...
VkAttachmentLoadOp mapVkLoadOp(LoadOp loadOp)
{
  switch (loadOp)
  {
    ...
    default:
    {
      EZ_ASSERT_DEV(false, "Unsupported LoadOp, convert to VkLoadOp failed.");
      return VK_ATTACHMENT_LOAD_OP_LOAD;
    }
  }
}
...
VkAttachmentStoreOp mapVkStoreOp(StoreOp storeOp)
{
  switch (storeOp)
  {
    ...
    default:
    {
      EZ_ASSERT_DEV(false, "Unsupported StoreOp, convert to VkStoreOp failed.");
      return VK_ATTACHMENT_STORE_OP_STORE;
    }
  }
}
...
VkImageType mapVkImageType(TextureType type)
{
  switch (type)
  {
    ...
    default:
    {
      EZ_ASSERT_DEV(false, "Unsupported TextureType, convert to VkImageType failed.");
      return VK_IMAGE_TYPE_2D;
    }
  }
}
...
VkImageViewType mapVkImageViewType(TextureType viewType)
{
  switch (viewType)
  {
    ...
    default:
    {
      EZ_ASSERT_DEV(false, "Unsupported TextureViewType, convert to VkImageViewType failed.");
      return VK_IMAGE_VIEW_TYPE_2D;
    }
  }
}
...
VkCommandBufferLevel mapVkCommandBufferLevel(CommandBufferType type)
{
  switch (type)
  {
    ...
    default:
    {
      EZ_ASSERT_DEV(false, "Unsupported CommandBufferType, convert to VkCommandBufferLevel failed.");
      return VK_COMMAND_BUFFER_LEVEL_SECONDARY;
    }
  }
}
...
VkDescriptorType mapVkDescriptorType(DescriptorType type)
{
  switch (type)
  {
    ...
    default:
    {
      EZ_ASSERT_DEV(false, "Unsupported DescriptorType, convert to VkDescriptorType failed.");
      return VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    }
  }
}
...
{
  ezUInt32 flags = 0U;
  if (HasFlag(colorMask, ColorMask::R))
    flags |= VK_COLOR_COMPONENT_R_BIT;
  if (HasFlag(colorMask, ColorMask::G))
    flags |= VK_COLOR_COMPONENT_G_BIT;
  if (HasFlag(colorMask, ColorMask::B))
    flags |= VK_COLOR_COMPONENT_B_BIT;
  if (HasFlag(colorMask, ColorMask::A))
...
VkShaderStageFlagBits mapVkShaderStageFlagBits(ShaderStageFlagBit stage)
{
  switch (stage)
  {
    ...
    default:
    {
      EZ_ASSERT_DEV(false, "Unsupported ShaderStageFlagBit, convert to VkShaderStageFlagBits failed.");
      return VK_SHADER_STAGE_VERTEX_BIT;
    }
  }
}
...
VkShaderStageFlags mapVkShaderStageFlags(ShaderStageFlagBit stages)
{
  ...
  if (HasFlag(stages, ShaderStageFlagBit::CONTROL))
    flags |= VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT;
  if (HasFlag(stages, ShaderStageFlagBit::EVALUATION))
    flags |= VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
  if (HasFlag(stages, ShaderStageFlagBit::GEOMETRY))
...
ezString mapVendorName(ezUInt32 vendorID)
{
  switch (vendorID)
  {
    case 0x1002:
  ...
  ezStringBuilder ret;
  ret.AppendFormat("Unknown VendorID {}", vendorID);
  return ret.GetData();
}
...
VkDeviceSize roundUp(VkDeviceSize numToRound, ezUInt32 multiple)
{
  return ((numToRound + multiple - 1) / multiple) * multiple;
}

bool isLayerSupported(const char* required, const ezDynamicArray<VkLayerProperties>& available)
{
...
bool isExtensionSupported(const char* required, const ezDynamicArray<VkExtensionProperties>& available)
{
