...
{
  _gpuCommandBuffer = EZ_DEFAULT_NEW(CCVKGPUCommandBuffer);
  ...
  ezUInt32 setCount = CCVKDevice::getInstance()->bindingMappingInfo().bufferOffsets.GetCount();
  _curGPUDescriptorSets.SetCount(setCount);
  _curVkDescriptorSets.SetCount(setCount);
  _curDynamicOffsetsArray.SetCount(setCount);
}
...
void CCVKCommandBuffer::doDestroy()
{
  ...
  {
    EZ_DEFAULT_DELETE(_gpuCommandBuffer);
    _gpuCommandBuffer = nullptr;
  }
}

void CCVKCommandBuffer::begin(RenderPass* renderPass, ezUInt32 subpass, Framebuffer* frameBuffer)
{
  if (_gpuCommandBuffer->began)
    return;
  ...
  //_curGPUDescriptorSets.assign(_curGPUDescriptorSets.GetCount(), nullptr);
  {
    for (ezUInt32 i = 0; i < _curGPUDescriptorSets.GetCount(); i++)
    {
      _curGPUDescriptorSets[i] = nullptr;
    }
  }

  //_curDynamicOffsetsArray.assign(_curDynamicOffsetsArray.GetCount(), {});
  {
    for (ezUInt32 i = 0; i < _curDynamicOffsetsArray.GetCount(); i++)
    {
      _curDynamicOffsetsArray[i] = {};
    }
  }
...
void CCVKCommandBuffer::end()
{
  ...
  _pendingQueue.PushBack(_gpuCommandBuffer->vkCommandBuffer);
...
void CCVKCommandBuffer::beginRenderPass(RenderPass* renderPass, Framebuffer* fbo, const Rect& renderArea, const Color* colors,
  float depth, ezUInt32 stencil, CommandBuffer* const* /*secondaryCBs*/, ezUInt32 secondaryCBCount)
{
  ...
  ezDynamicArray<VkClearValue>& clearValues = gpuRenderPass->clearValues;
  bool depthEnabled = gpuRenderPass->depthStencilAttachment.format != Format::UNKNOWN;
  ezUInt32 attachmentCount = depthEnabled ? clearValues.GetCount() - 1 : clearValues.GetCount();

  for (ezUInt32 i = 0U; i < attachmentCount; ++i)
  {
  ...
  passBeginInfo.clearValueCount = clearValues.GetCount();
  passBeginInfo.pClearValues = clearValues.GetData();
...
void CCVKCommandBuffer::endRenderPass()
{
  ...
  ezUInt32 colorAttachmentCount = _curGPUFBO->gpuRenderPass->colorAttachments.GetCount();
...
void CCVKCommandBuffer::bindDescriptorSet(ezUInt32 set, DescriptorSet* descriptorSet, ezUInt32 dynamicOffsetCount, const ezUInt32* dynamicOffsets)
{
  EZ_ASSERT_DEV(_curGPUDescriptorSets.GetCount() > set, "Invalid set index");
  ...
  if (dynamicOffsetCount)
  {
    ...
    {
      _curDynamicOffsetsArray[set].SetCount(dynamicOffsetCount);
      for (ezUInt32 i = 0; i < dynamicOffsetCount; i++)
      {
        _curDynamicOffsetsArray[set][i] = *(dynamicOffsets + i * (ezUInt32)sizeof(dynamicOffsets[0]));
      }
    }
    if (set < _firstDirtyDescriptorSet)
      _firstDirtyDescriptorSet = set;
  ...
  {
    //_curDynamicOffsetsArray[set].assign(_curDynamicOffsetsArray[set].GetCount(), 0);
    {
      for (ezUInt32 i = 0; i < _curDynamicOffsetsArray[set].GetCount(); i++)
      {
        _curDynamicOffsetsArray[set][i] = 0;
      }
    }
  }
}
...
void CCVKCommandBuffer::bindInputAssembler(InputAssembler* ia)
{
  ...
  if (_curGPUInputAssember != gpuInputAssembler)
  {
    ...
    ezUInt32 vbCount = gpuInputAssembler->gpuVertexBuffers.GetCount();
    if (gpuInputAssembler->vertexBuffers.GetCount() < vbCount)
    {
      gpuInputAssembler->vertexBuffers.SetCount(vbCount);
      gpuInputAssembler->vertexBufferOffsets.SetCount(vbCount);
    }
    ...
    vkCmdBindVertexBuffers(_gpuCommandBuffer->vkCommandBuffer, 0, vbCount,
      gpuInputAssembler->vertexBuffers.GetData(), gpuInputAssembler->vertexBufferOffsets.GetData());
...
{
  if (!ezMath::IsEqual(_curDynamicStates.lineWidth, width, ezMath::SmallEpsilon<float>()))
  {
...
{
  if (!ezMath::IsEqual(_curDynamicStates.depthBiasConstant, constant, ezMath::SmallEpsilon<float>()) ||
      !ezMath::IsEqual(_curDynamicStates.depthBiasClamp, clamp, ezMath::SmallEpsilon<float>()) ||
      !ezMath::IsEqual(_curDynamicStates.depthBiasSlope, slope, ezMath::SmallEpsilon<float>()))
  {
...
{
  if (!ezMath::IsEqual(_curDynamicStates.blendConstant.x, constants.x, ezMath::SmallEpsilon<float>()) ||
      !ezMath::IsEqual(_curDynamicStates.blendConstant.y, constants.y, ezMath::SmallEpsilon<float>()) ||
      !ezMath::IsEqual(_curDynamicStates.blendConstant.z, constants.z, ezMath::SmallEpsilon<float>()) ||
      !ezMath::IsEqual(_curDynamicStates.blendConstant.w, constants.w, ezMath::SmallEpsilon<float>()))
  {
...
{
  if (!ezMath::IsEqual(_curDynamicStates.depthMinBounds, minBounds, ezMath::SmallEpsilon<float>()) ||
      !ezMath::IsEqual(_curDynamicStates.depthMaxBounds, maxBounds, ezMath::SmallEpsilon<float>()))
  {
...
void CCVKCommandBuffer::setStencilWriteMask(StencilFace face, ezUInt32 mask)
{
  ...
  {
    if (front.writeMask == mask && back.writeMask == mask)
      return;
...
void CCVKCommandBuffer::setStencilCompareMask(StencilFace face, ezUInt32 reference, ezUInt32 mask)
{
  DynamicStencilStates& front = _curDynamicStates.stencilStatesFront;
  DynamicStencilStates& back = _curDynamicStates.stencilStatesBack;
  if (face == StencilFace::ALL)
  {
    if (front.reference == reference && back.reference == reference &&
        front.compareMask == mask && back.compareMask == mask)
      return;
  ...
  {
    if (front.writeMask == mask && front.reference == reference)
      return;
    front.writeMask = mask;
  ...
  {
    if (back.writeMask == mask && back.reference == reference)
      return;
    back.writeMask = mask;
...
{
  if (_firstDirtyDescriptorSet < _curGPUDescriptorSets.GetCount())
  {
  ...
  {
    ezUInt32 drawInfoCount = gpuIndirectBuffer->range / gpuIndirectBuffer->gpuBuffer->stride;
    CCVKGPUDevice* gpuDevice = CCVKDevice::getInstance()->gpuDevice();
  ...
  {
    ezUInt32 instanceCount = ezMath::Max(info.instanceCount, 1U);
    ...
    {
      ezUInt32 indexCount = hasIndexBuffer ? info.indexCount : info.vertexCount;
...
void CCVKCommandBuffer::execute(CommandBuffer* const* cmdBuffs, ezUInt32 count)
{
  if (!count)
    return;
  _vkCommandBuffers.SetCount(count);

  ezUInt32 validCount = 0U;
  for (ezUInt32 i = 0U; i < count; ++i)
  {
    auto* cmdBuff = static_cast<CCVKCommandBuffer*>(cmdBuffs[i]);
    if (!cmdBuff->_pendingQueue.IsEmpty())
    {
      _vkCommandBuffers[validCount++] = cmdBuff->_pendingQueue.PeekFront();
  ...
  if (validCount)
  {
    vkCmdExecuteCommands(_gpuCommandBuffer->vkCommandBuffer, validCount,
      _vkCommandBuffers.GetData());
  }
}

void CCVKCommandBuffer::updateBuffer(Buffer* buffer, const void* data, ezUInt32 size)
{
...
void CCVKCommandBuffer::copyBuffersToTexture(const uint8_t* const* buffers, Texture* texture, const BufferTextureCopy* regions, ezUInt32 count)
{
...
void CCVKCommandBuffer::blitTexture(Texture* srcTexture, Texture* dstTexture, const TextureBlit* regions, ezUInt32 count, Filter filter)
{
  ...
  vkCmdBlitImage(_gpuCommandBuffer->vkCommandBuffer,
    ...
    count, _blitRegions.GetData(), VK_FILTERS[ToNumber(filter)]);
...
void CCVKCommandBuffer::bindDescriptorSets(VkPipelineBindPoint bindPoint)
{
  ...
  ezDynamicArray<ezUInt32>& dynamicOffsetOffsets = pipelineLayout->dynamicOffsetOffsets;
  ezUInt32 descriptorSetCount = pipelineLayout->setLayouts.GetCount();
  _curDynamicOffsets.SetCount(pipelineLayout->dynamicOffsetCount);

  ezUInt32 dirtyDescriptorSetCount = descriptorSetCount - _firstDirtyDescriptorSet;
  for (ezUInt32 i = _firstDirtyDescriptorSet; i < descriptorSetCount; ++i)
  {
    ...
    ezUInt32 count = dynamicOffsetOffsets[i + 1] - dynamicOffsetOffsets[i];
    //EZ_ASSERT_DEV(_curDynamicOffsetCounts[i] >= count, "missing dynamic offsets?");
    count = ezMath::Min(count, _curDynamicOffsetsArray[i].GetCount());
    if (count > 0)
      memcpy(&_curDynamicOffsets[dynamicOffsetOffsets[i]], _curDynamicOffsetsArray[i].GetData(), count * sizeof(ezUInt32));
  }

  ezUInt32 dynamicOffsetStartIndex = dynamicOffsetOffsets[_firstDirtyDescriptorSet];
  ezUInt32 dynamicOffsetEndIndex = dynamicOffsetOffsets[_firstDirtyDescriptorSet + dirtyDescriptorSetCount];
  ezUInt32 dynamicOffsetCount = dynamicOffsetEndIndex - dynamicOffsetStartIndex;
  vkCmdBindDescriptorSets(_gpuCommandBuffer->vkCommandBuffer,
    ...
    dynamicOffsetCount, _curDynamicOffsets.GetData() + dynamicOffsetStartIndex);
...
void CCVKCommandBuffer::pipelineBarrier(const GlobalBarrier* barrier, const TextureBarrier* const* textureBarriers, const Texture* const* textures, ezUInt32 textureBarrierCount)
{
  VkPipelineStageFlags srcStageMask = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
  VkPipelineStageFlags dstStageMask = VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT;
  VkMemoryBarrier const* pMemoryBarrier = nullptr;
  ezUInt32 memoryBarrierCount = 0U;
  ...
  {
    _imageMemoryBarriers.SetCount(textureBarrierCount);
    pImageMemoryBarriers = _imageMemoryBarriers.GetData();

    for (ezUInt32 i = 0U; i < textureBarrierCount; ++i)
    {
      ...
      {
        gpuTexture->currentAccessTypes.SetCount(gpuBarrier->barrier.nextAccessCount);
        for (ezUInt32 j = 0u; j < gpuBarrier->barrier.nextAccessCount; j++)
        {
          gpuTexture->currentAccessTypes[j] = *(gpuBarrier->barrier.pNextAccesses + j);
        }
      }
    }
  }
